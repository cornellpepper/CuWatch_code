    ...
    <html>
        <head>
            ...
            <script>
                ...
                // Fetch updated rate, muon_count, and iteration_count from the server every 5 seconds
                function fetchData() {
                    fetch('/data')
...
def submit(request):
    ...
    try:
        # Get the new threshold value from the form and update it
        threshold = int(request.form['threshold'])  # Convert to integer
        
        # Redirect to the main page to avoid the form re-submission prompt
